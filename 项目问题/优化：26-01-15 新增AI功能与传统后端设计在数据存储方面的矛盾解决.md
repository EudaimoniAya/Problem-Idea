...
【注：此处因为对于PostgreSQL的不熟悉，以及缺少Docker的项目实操，无法一次性解决该问题。所以把它做一个契机，沉下心来多花三四天去深入地理解并配合一些小实战，这段时间整理的笔记为[[PostgreSQL和pgvector]]（01-15）】
...
```bash
docker run -d --name postgres-pgvector -e POSTGRES_PASSWORD=my_password -e POSTGRES_DB=backend_test -p 55432:5432 ankane/pgvepostgres-pgvectorctor:latest
```
然后就创建了一个容器，它拉取了官方仓库的镜像，让名为`postgres-pgvector`的容器有了一个外部端口为55432，对应内部端口为5432的端口映射。然后运行容器后，在pgAdmin4中创建端口为55432的新服务器，它就能够和容器内部的扩展了`pgvector`的PostgreSQL关联并操纵，并在新增扩展中能够添加`vetor`属性以支持向量存储了。
