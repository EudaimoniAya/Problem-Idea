...
2. 使用`asyncio.run()`来运行一个**入口协程**（这是Python 3.7+推荐的方式，**它会创建事件循环，运行协程**，然后关闭循环。但是`asyncio.run()`**只适用于顶层入口**，而在这个入口协程内部，可以通过 `asyncio.create_task()` 创建多个任务，让它们并发执行）。
3. 使用`asyncio.create_task()`来**将协程包装成任务**，并在事件循环中调度（这需要在事件循环中运行），该方法支持并发执行多个任务，比如同时等待数据库查询和HTTP请求。
