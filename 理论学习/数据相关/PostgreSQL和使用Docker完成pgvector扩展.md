...
* `-d`：表示**分离模式（detached mode），让容器在后台执行，不会阻塞当前窗口**（`docker run`命令会导致命令行的持续占用，**通常都要加上这个参数**）。运行后会返回一个容器ID，后续的容器日志不会打印在控制台。
  如果需要重新让容器在控制台打印日志，那就可以使用`docker attach`命令，它能够**将容器主进程的标准输出`stdout`和标准错误`stderr`重定向到当前终端**。如果主进程非交互式那么再次连接会继续阻塞终端，如果是用`-it`启动的交互式shell，如`bash`，用`docker attach`就可以重新进入shell。
