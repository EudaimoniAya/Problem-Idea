...
Python并发能力弱，源于它有一把GIL锁限制了多核下的一个线程并发（疑似3.14版本解决），所以突破瓶颈的一个方式就是基于协程异步 => FastAPI
...
WSGI是同步的web协议（一次请求对应一个线程/进程）、**ASGI**（Asynchronous Server Gateway Interface，**异步服务器网关接口**）是包含异步服务的web协议，而Starlette是基于ASGI框架的。
ASGI定义了`http`、`websocket`、`lifespan`三种作用域（scope），每个连接由**异步的可接收事件和发送事件**处理，这使得应用能够“等待”外部资源时不阻塞其他连接。
...
**Content-type**，相当重要，用于告诉服务器**请求体中数据的类型和格式**（`json`、`urlencoded`）。服务器接收到请求时，先去看请求头中的`Content-type`，才能正确地解析请求体中数据。
...
目前市面上大部分公司开发人员使用的接口实现规范主要有：**restful**、RPC（远程过程调用）
...
当客户端发送一个**http请求**后，先通过uvicorn接收（uvicorn相当于在FastAPI框架外面**封装了一个socket**），然后把请求处理好的数据再交给Web应用程序或Web框架（由路由系统映射函数完成）。也就是到了Web框架这一层级时，一定要有一个路由映射函数的机制，即对于不同请求对应的不同路径，将其交给不同的函数处理。

学习web框架必须记住这个流程：（uvicorn封装 ->）路由系统 -> 逻辑函数（请求参数 -> 响应方式），其中，在响应过程中需要数据库操作即ORM。另外模板引擎、中间件也在其中作用

### ASGI接口层的具体实现：ASGI服务器Uvicorn
ASGI服务器是实现了ASGI规范的服务端程序，它负责监听网络端口，接收客户端连接，将每个连接根据协议（HTTP/WebSocket等）转换为一组ASGI定义的”作用域“（scope）和”事件“（event）。**并调用编写的Python应用（一个符合ASGI接口的`Callable`对象），并把应用的返回结果通过底层网络发送回客户端**。常见的ASGI服务器有：**Uvicorn**、Daphne、Hypercorn
其中，作用域`scope`是一个**只读**的Python字典，包含请求的元数据：
```python
{
    "type": "http",    # 协议类型：http / websocket
    "asgi": {"version": "3.0"},
    "http_version": "1.1",
    "method": "GET",
    "scheme": "http",
...
```
FastAPI会从这里找到**当前请求**的类型`type`、方法`method`、头部`headers`、协议版本（ASGI版本`asgi`、HTTP版本`http_version`）等信息。然后FastAPI会根据`scope`的`path`进行路由匹配（不同请求的字段不同，但均遵循ASGI规范）
之所以被称为作用域，是因为**它描述了`await app(scope, receive, send)`的作用范围（上下文）**：当uvicorn接收到HTTP请求时，会根据该请求生成一个仅针对该请求的scope，并通过调用`app`把事件循环控制权交给FastAPI。FastAPI应用在执行其业务逻辑时，可能会多次执行`receive`和`send`函数，因为可能请求报文很大，或者要发送的文件很多（**通过这两个函数传输的字典类型数据被称为ASGI事件**）。此时`scope`**作为元数据，完成该请求期间的所有ASGI事件都会以它为上下文**。
#### Uvicorn的工作机制
对于一个极简的FastAPI应用框架：
```python
...
```
当`import uvicorn`时**不会启动任何服务器**，它只把uvicorn的代码加载进内存，真正让服务器跑起来有两种方式：
* 命令行执行：`uvicorn main:app --host 0.0.0.0 --port 8000`
* 代码中调用：`uvicorn.run("main:app", host="0.0.0.0", port=8000)`
调用`uvicorn.run()`时：
1. 解释器会读取传入的参数、环境变量、配置文件等（如果`reload=True`就会**额外启动一个监视进程，在文件变化时自动重启**；如果`workers > 1`则会启用生产环境常用的多进程模式）。
2. 根据传入的字符串`"main:app"`，**动态导入`main`模块，从中取出`app`对象**。字符串格式为`<模块名>:<应用对象名>`，其中应用对象名任意，模块名必须在Python的**模块搜索路径`sys.path`中能够找到**，不一定必须在顶层模块（**但不支持相对路径和变量**），比如`src.main.app`就可以在`arc/main.py`下找到`app`
   uvicorn要求传入的必须是一个可调用的对象，接受 `(scope, receive, send)` 参数，且通常是一个异步函数或实现了 `__call__` 的类实例。所以**传入的可以不是`"main:app"`，而是已经事先定义好的`app`对象**
3. uvicorn会在**Python进程内部**会创建一个`uvicorn.Server`类的实例，启动`asyncio`事件循环（如果安装了`uvloop`则会用`uvloop`替换，以获得更高性能），然后**将服务器实例作为异步任务添加到事件循环中进行**
4. 在执行服务器实例时，会调用底层socket API，创建TCP服务器绑定到指定的host和port，准备接受新连接
5. 当连接被发起时，uvicorn服务器会接收该TCP连接，根据协议初步解析，创建一个ASGI`scope`字典（包含请求方法、路径、头信息、协议类型等）并**构造两个异步的“发送”和“接收”函数（对应 ASGI 接口的 `send` 和 `receive`），并通过`await app(scope, receive, send)`将控制权交给FastAPI项目，并等待它完成整个请求处理（中间可能会反复调用`send`和`receive`，因为数据可能需要多次才能传送完毕）**：
   * 接收函数`receive`**根据协议，将TCP字节流还原为应用层数据，并包装成FastAPI可读的标准ASGI事件**
   * 发送函数`send`**把FastAPI返回的数据转换成字节流发回客户端**
   **这两个接口由FastAPI主动调用**，`receive()`调用后FastAPI会等待并返回请求体的下一块数据；`send(event)`会将应用生成的响应事件发送到TCP连接
所以在通过http向FastAPI应用进行请求时，请求数据将会从传输层到应用层的应用中，更为细化则有：**HTTP协议层 -> ASGI接口层 -> Web框架层**（ASGI应用，如FastAPI），然后进入代码的业务逻辑。其中**HTTP层和ASGI有严格规定或是社区规范**，而Web框架层、业务逻辑层则是**属于软件设计模式（MVC、MVT）的实践**，不同框架虽然内部实践不同，但都遵循类似的请求-响应流程。
...
**这类似计算机网络的特定主机路由。因为是最具象的，所以后缀是/32（以ipv4为例），根据最大长度匹配原则和路由器IP存放顺序由具体到抽象，所以这些特定主机路由是最先访问的。使用FastAPI开发时，对于路由的管理也可以参考这样，把最重要的一些特殊方法，如对于特殊用户id返回管理员身份而不是视为普通用户**。
利用这个机制，而不是之间把判断id是否为管理员放进方法的实现中，**减少了每次普通用户访问时进行的多余的判断**（因为本来就要从上至下查找接口，如果执行到了那个方法的话，说明一定是普通用户），不仅让效率上升还让代码整洁美观
...
同样，以上的这些所有fastapi测试文档中测试的请求，都可以通过postman发送，但是它是一种最原始最客观的发送方式。fastapi文档是基于后端代码实现的，所以借助浏览器发送规范请求的它特别方便测试，但是现实情况不是所有前端开发者都是懂fastapi后端的规范，此时postman就是一个很客观的，模拟对后端一无所知的前端开发者的测试工具。
...
这就是所谓的“声明式”编程：你**声明参数应该是什么，而由框架来负责如何获取它，无需关心怎么做**。这是现代框架（如FastAPI、React、Kubernetes）的核心设计哲学。
...
原理：FastAPI在启动时扫描所有的路由函数，解析所有函数的参数。Request直接来自FastAPI的底层框架Starlette，**它被FastAPI直接识别为”要注入的对象“**，背后有`receive`方法作用，用于接收请求数据块。T是一个特殊类型，因为每个请求只有一个`Request`对象，所以它完美地符合单例模式，且`Request()`需要**运行时创建（等请求来了才能获取到），所以函数定义时传入它没有任何意义**。
